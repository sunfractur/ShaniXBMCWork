...
def decrypt_vipracing(page_url):
	...
	str_pattern='c=\"(.*?)\"'
	meta=re.compile(str_pattern).findall(html)[0] 
	str_pattern='x\(\"(.*?)\"\)'
...
#	final_rtmp=' token=$doregex[tok] pageUrl=http://www.direct2watch.com/ live=1 timeout=10</link>
	...
	final_rtmp='%s app=%s playpath=%s swfUrl=http://www.direct2watch.com/player/player_embed_iguide.swf token=%s live=1 timeout=10 pageUrl=%s'%(streamer,app,file,token,url)
	return final_rtmp
